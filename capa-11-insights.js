...
(function () {
  ...
  var abierta = false;
  var fab = null;
  var vistaViva = ruta();     // 28/07: en qué vista estábamos, para borrar su hilo al salir
  var cerrarHoja = null;      // cierre de la hoja abierta, visible desde el módulo
  ...
  /* Vistas cuyo "Iniciar" responde el propio panel, sin Gemini. La lista la
     manda el CORE (informeVistas) para no tenerla escrita en dos sitios; si
     un CORE viejo no la manda, se asume que ninguna la tiene. */
  function conInforme(vista) {
    var l = (estado && estado.informeVistas) || [];
    for (var i = 0; i < l.length; i++) if (l[i] === vista) return true;
    return false;
  }
  ...
  function montarFab() {
    ...
    /* 06/08/2026 · rol INVITADO: el robot solo si tiene la pastilla
       "Analizar con IA" (o el candado Sin IA sin poner). permOk() dice que sí
       a todo para los demás roles, así que esto no cambia nada para ellos. */
    if (typeof permOk === 'function' && !permOk('ia.usar')) return quitarFab();

    pedirEstado().then(function (e) {
      if (ruta() !== r) return;                       // se cambió de vista mientras respondía
      /* 28/07/2026 · HÍBRIDO: "Iniciar" lo arma el CORE con los datos del
         panel, sin Gemini, así que el robot sirve aunque no haya clave. Solo
         se esconde en las vistas SIN informe propio, donde sin clave no
         haría nada. */
      if (!e.configurada && !conInforme(r) && !esDev()) return quitarFab();
  ...
  function abrir(vista) {
    ...
    if (hilos[vista].length) {
      ...
      pintarAviso(body, conInforme(vista)
        ? ('Envía «Iniciar» para el informe de ' + (VISTAS[vista] || 'esta vista') +
           ' con las cifras del panel, al instante y sin IA' +
           ((estado && estado.configurada)
              ? '. O borra la palabra y pregúntame lo que necesites: eso sí va a la IA.'
              : '. Para preguntar por escrito falta la clave de Gemini (Configuración → Avanzado).'))
        : ('Envía «Iniciar» para el análisis completo de ' + (VISTAS[vista] || 'esta vista') +
  ...
  /* La saturación de Google (HTTP 503) se va sola en un rato: el usuario no
     debería tener que volver a escribir la pregunta. Cuando el CORE marca el
     fallo como reintentable, la burbuja de error trae su propio botón. */
  function pintarError(body, msg, reintentar) {
    var el = nodo('<div class="ia-msg ia-err"></div>');
    el.appendChild(document.createTextNode(String(msg)));
    if (typeof reintentar === 'function') {
      var b = nodo('<button class="ia-err-re" type="button">Reintentar</button>');
      ...
      .then(function (r) {
        ...
        if (!r || r.ok === false) {
          pintarError(body, (r && r.msg) || 'No se pudo generar el análisis.',
            (r && r.reintentable) ? function () { consultar(vista, body, pregunta, input); } : null);
          return;
        }
        var meta = (r.modo === 'panel')
          ? ('Informe del panel · sin IA · ' + (r.generado || ''))
          : (r.modelo ? (r.modelo + ' · ' + (r.generado || '')) : '');
        var burbuja = pintar(body, 'ia', r.texto, meta);
        /* Puede haberse cambiado de vista mientras Gemini respondía: ahí el
           hilo ya se borró a propósito y no se resucita. */
        if (hilos[vista]) hilos[vista].push({ rol: 'ia', texto: r.texto });
  ...
  /* Qué está mirando el usuario ahora mismo: buscador, filtros y pestañas.
     Se lee del DOM porque es donde vive la verdad de la vista.
     28/07/2026: antes solo tres vistas mandaban contexto. La consecuencia
     real: se preguntaba por alguien que estaba escrito en el buscador de
     Líderes y al backend no le llegaba ni el nombre ni el recorte. */
  var CTX = {
    bd:             { q: '#bd-q',  cuenta: '#bd-count', chips: '#bd-chips .bd-chip.active', otros: '#bd-chips-int .bd-chip-int.active' },
    lideres:        { q: '#ld-q',  cuenta: '#ld-count' },
    eventos:        { q: '#ev-q',  cuenta: '#ev-count',  chips: '#ev-chips .bd-chip.active' },
    compromisos:    { q: '#cm-q',  cuenta: '#cm-count',  chips: '#cm-chips .bd-chip.active' },
    solicitudes:    { q: '#sl-q',  cuenta: '#sl-count',  chips: '#sl-chips .bd-chip.active' },
    notificaciones: { q: '#nt-q',  cuenta: '#nt-count',  chips: '#nt-chips .bd-chip.active' },
    votacion:       { seg: '#voto-tabs .seg-b.active' }
  };
  ...
  /* ------------------------------------------------------------
     28/07/2026 · LOS FILTROS DE VERDAD
     Antes solo viajaba una FRASE ("filtro: Flandes · búsqueda: oscar") y el
     CORE calculaba los conteos sobre TODAS las filas. El usuario filtraba
     Análisis → Servicios por profesional y servicio, preguntaba, y el robot
     le hablaba de todo. No era el prompt: el dato filtrado no existía en el
     envío.
     Ahora, además de la frase, va el filtro ESTRUCTURADO (`f`) y cuántas
     filas hay en pantalla (`enPantalla`), que el CORE usa para rehacer el
     recorte y para avisar si su cuenta no coincide con la de la pantalla.
     Se leen las variables de app.js por su nombre (mismo ámbito global) y
     sus propias funciones de filtro cuando existen: así no hay dos verdades.
     NO se toca app.js.
     ------------------------------------------------------------ */

  function num(v) { return (typeof v === 'number' && isFinite(v) && v >= 0) ? v : null; }
  function deSet(s) { try { return s && s.forEach ? [].slice.call(setArr(s)) : []; } catch (e) { return []; } }
  function setArr(s) { var a = []; s.forEach(function (x) { a.push(x); }); return a; }
  function valDe(sel) { var el = document.querySelector(sel); return el ? String(el.value || '').trim() : ''; }
  function chipDe(sel) { var el = document.querySelector(sel); return el ? String((el.dataset && el.dataset.f) || '').trim() : ''; }

  /* Pestaña de BD → código estable, para que el CORE no adivine cuál de las
     pastillas es el municipio de la campaña (esa etiqueta cambia por campaña). */
  function codChipBd(lbl) {
    var t = String(lbl || 'Todos').trim();
    var b = t.toLowerCase();
    if (!t || b === 'todos') return 'todos';
    if (b === 'inactivos') return 'inactivos';
    if (b === 'sin consultar') return 'sinconsultar';
    if (b === 'a la espera') return 'espera';
    if (b === 'por confirmar') return 'porconfirmar';
    if (b === 'otros') return 'otros';
    return 'municipio';
  }
  ...
  function filtrosDe(vista) {
    ...
    if (vista === 'bd') {
      var lbl = (typeof bdActiveFilter === 'function') ? String(bdActiveFilter() || 'Todos') : chipDe('#bd-chips .bd-chip.active');
      f = {
        chip: codChipBd(lbl),
        chipLabel: lbl,
        intencion: (typeof BD_INT !== 'undefined') ? deSet(BD_INT) : [],
        lider: (typeof BD_LID !== 'undefined' && BD_LID != null) ? String(BD_LID) : '',
        q: valDe('#bd-q')
      };
      if (typeof BD_FILT !== 'undefined' && BD_FILT) n = BD_FILT.length;

    } else if (vista === 'dashboard') {
      f = {
        intencion: (typeof DASH_INT !== 'undefined') ? deSet(DASH_INT) : [],
        soloPuesto: (typeof DASH_SOLO_PUESTO !== 'undefined') ? !!DASH_SOLO_PUESTO : false,
        q: (typeof DASH_Q !== 'undefined') ? String(DASH_Q || '').trim() : ''
      };
      if (typeof dashFilteredRows === 'function') { try { n = dashFilteredRows().length; } catch (e) {} }

    } else if (vista === 'votacion') {
      f = {
        tab: (typeof VOTO_TAB !== 'undefined') ? String(VOTO_TAB || '') : '',
        q: (typeof VOTO_Q !== 'undefined') ? String(VOTO_Q || '').trim() : ''
      };
      /* La pantalla lista ETIQUETAS (puestos, mesas, líderes), no votos: no
         se manda conteo o el CORE creería que su cuenta de votos no cuadra. */

    } else if (vista === 'lideres') {
      f = { q: valDe('#ld-q') };
      if (typeof LD_FILT !== 'undefined' && LD_FILT) n = LD_FILT.length;

    } else if (vista === 'agenda') {
      var rango = {};
      if (typeof AG_CUR !== 'undefined' && AG_CUR) {
        var v = (typeof AG_VIEW !== 'undefined') ? String(AG_VIEW) : 'mes';
        rango.y = AG_CUR.getFullYear();
        if (v !== 'anio') rango.m = AG_CUR.getMonth() + 1;
        if (v === 'dia') rango.d = AG_CUR.getDate();
      }
      f = {
        rango: rango,
        vista: (typeof AG_VIEW !== 'undefined') ? String(AG_VIEW) : '',
        periodo: txt('#ag-title'),
        estados: (typeof AG_EST !== 'undefined') ? deSet(AG_EST) : [],
        q: (typeof AG_Q !== 'undefined') ? String(AG_Q || '').trim() : ''
      };
      if (typeof AG_ALL !== 'undefined' && typeof agEnRango === 'function' && typeof agMatchesQ === 'function') {
        try {
          n = AG_ALL.filter(function (r) {
            if (!agEnRango(r) || !agMatchesQ(r)) return false;
            if (typeof AG_EST !== 'undefined' && AG_EST.size && typeof agEstadoNormJs === 'function') return AG_EST.has(agEstadoNormJs(r.estado));
            return true;
          }).length;
        } catch (e) {}
      }

    } else if (vista === 'eventos' || vista === 'compromisos' || vista === 'solicitudes' || vista === 'notificaciones') {
      var pre = { eventos: 'ev', compromisos: 'cm', solicitudes: 'sl', notificaciones: 'nt' }[vista];
      f = { estado: chipDe('#' + pre + '-chips .bd-chip.active'), q: valDe('#' + pre + '-q') };
      var arr = (vista === 'eventos') ? (typeof EV_FILT !== 'undefined' ? EV_FILT : null)
        : (vista === 'compromisos') ? (typeof CM_FILT !== 'undefined' ? CM_FILT : null)
        : (vista === 'solicitudes') ? (typeof SL_FILT !== 'undefined' ? SL_FILT : null)
        : (typeof NT_FILT !== 'undefined' ? NT_FILT : null);
      if (arr) n = arr.length;

    } else if (vista === 'analisis') {
      if (typeof AN === 'undefined' || !AN) return { f: null, n: null };
      var tab = String(AN.tab || 'servicios');
      f = { tab: tab };
      if (AN.sFiltro) f.s = {
        est: deSet(AN.sFiltro.est), medio: deSet(AN.sFiltro.medio),
        servicio: String(AN.sFiltro.servicio || ''), prof: String(AN.sFiltro.prof || ''),
        q: String(AN.sFiltro.q || ''), eje: String(AN.sFiltro.eje || '')
      };
      if (AN.iFiltro) f.i = { est: deSet(AN.iFiltro.est), cat: deSet(AN.iFiltro.cat), q: String(AN.iFiltro.q || '') };
      if (AN.cFiltro) f.c = { est: String(AN.cFiltro.est || 'TODOS'), cat: String(AN.cFiltro.cat || 'TODAS'), q: String(AN.cFiltro.q || '') };
      try {
        if (tab === 'servicios' && AN.serv && typeof anServFiltradas === 'function') n = anServFiltradas().length;
        else if (tab === 'ideas' && AN.ideas && typeof anIdeasFiltradas === 'function') n = anIdeasFiltradas().length;
        else if (tab === 'comercio' && AN.com && typeof anComFiltrados === 'function') n = anComFiltrados().length;
      } catch (e) {}
    }
  ...
  function contextoDe(vista) {
    var c = {};
    try {
      var m = CTX[vista], partes = [];
      if (m) {
        if (m.chips) {
          var ch = document.querySelector(m.chips);
          if (ch && ch.dataset.f && ch.dataset.f !== 'Todos' && ch.dataset.f !== 'Todas') partes.push('filtro: ' + ch.dataset.f);
        }
        if (m.otros) {
          var ints = [].slice.call(document.querySelectorAll(m.otros)).map(function (x) { return x.dataset.int || ''; }).filter(Boolean);
          if (ints.length) partes.push('intención: ' + ints.join(', '));
        }
        if (m.q) {
          var q = document.querySelector(m.q);
          if (q && String(q.value || '').trim()) partes.push('búsqueda escrita: ' + String(q.value).trim());
        }
        if (m.cuenta && txt(m.cuenta)) partes.push('en pantalla: ' + txt(m.cuenta));
        if (m.seg) {
          var sg = document.querySelector(m.seg);
          if (sg && sg.dataset.t) partes.push('pestaña: ' + sg.dataset.t);
        }
      }
      if (vista === 'analisis') {
      ...
      if (vista === 'agenda') {
        var t = txt('#ag-title');
        if (t) partes.push('periodo en pantalla: ' + t);
      }
      if (partes.length) c.filtro = partes.join(' · ');
    } catch (e) {}
    /* El filtro estructurado va aparte y NUNCA puede tumbar la consulta:
       si algo de app.js no está donde se espera, se manda sin él y el CORE
       se comporta como antes. */
    try {
      var r = filtrosDe(vista);
      if (r && r.f) c.f = r.f;
      if (r && r.n !== null && r.n !== undefined) c.enPantalla = r.n;
      if (c.f && c.f.tab && !c.tab) c.tab = c.f.tab;
    } catch (e) {}
    return c;
  }
  ...
  function montarConfig() {
    ...
    pedirEstado().then(function (e) {
      ...
      var html =
        ...
        (e.configurada ? 'Clave de Gemini guardada' : 'FALTA la clave de Gemini: «Iniciar» funciona igual, pero no se puede preguntar por escrito') +
        '</span></div>' +
        ...
        '<div class="cfg-field full"><label>Modelos de respaldo</label>' +
        '<input class="input" id="ia-cfg-alt" value="' + limpio(e.modeloAlt || '') + '" placeholder="uno o dos, separados por comas" autocomplete="off" />' +
        '<div class="ia-cfg-fila"><button class="btn btn-quiet" id="ia-cfg-alt-add" type="button">Añadir el de la lista</button></div>' +
        '<div class="cfg-hint">Cuando Google responde <b>HTTP 503</b> (el modelo está saturado), el robot reintenta y luego salta a estos, en orden y sin preguntar. Sácalos del botón "Ver disponibles": los nombres caducan. Vacío = sin respaldo.</div></div>' +
        '<div class="cfg-field full"><label>Segundos de insistencia</label>' +
        '<input class="input" id="ia-cfg-espera" type="number" min="1" max="60" step="1" value="' + (parseInt(e.espera, 10) || 8) + '" />' +
        '<div class="cfg-hint">Cuánto insiste cuando Google está saturado, antes de rendirse. No cuenta lo que tarda el análisis bueno. Subirlo solo deja al usuario mirando la pantalla más rato.</div></div>' +
      ...
      /* Pasa el modelo elegido en "Ver disponibles" a la lista de respaldos,
         para no tener que escribir el nombre a mano y equivocarse. */
      var addAlt = sec.querySelector('#ia-cfg-alt-add');
      if (addAlt) addAlt.addEventListener('click', function () {
        if (!modsSel || modsSel.style.display === 'none' || !modsSel.value) {
          return toast('Pulsa "Ver disponibles" y elige uno primero.', 'err');
        }
        var campo = sec.querySelector('#ia-cfg-alt');
        var ya = campo.value.split(/[,;]+/).map(function (x) { return x.trim().toLowerCase(); });
        var nuevo = modsSel.value.trim();
        if (!nuevo || ya.indexOf(nuevo.toLowerCase()) >= 0) return;
        if (nuevo.toLowerCase() === sec.querySelector('#ia-cfg-modelo').value.trim().toLowerCase()) {
          return toast('Ese ya es el modelo principal.', 'err');
        }
        campo.value = campo.value.trim() ? (campo.value.trim().replace(/[,\s]+$/, '') + ', ' + nuevo) : nuevo;
      });
      ...
      if (guardar) guardar.addEventListener('click', function () {
        ...
        var alt = sec.querySelector('#ia-cfg-alt').value;
        var esp = sec.querySelector('#ia-cfg-espera').value;
        guardar.disabled = true;
        api('priv.iaClave', {}, 'POST', { caller: yo(), clave: k, modelo: m, enfoque: enf, textoLibre: tl, pensar: pn, modeloAlt: alt, espera: esp }, { silent: true })
          .then(function (r) {
            guardar.disabled = false;
            toast((r && r.msg) || 'Listo.', (r && r.ok) ? '' : 'err');
            if (r && r.ok) { estado = null; sec.querySelector('#ia-cfg-key').value = ''; }
            /* La pastilla de arriba se quedaba con el texto viejo ("FALTA la
               clave") hasta salir y volver a entrar a Configuración. */
            var past = sec.querySelector('.cfg-est');
            if (past && r && r.configurada === true) {
              past.className = 'cfg-est ok';
              past.textContent = 'Clave de Gemini guardada';
            }
          })
  ...
  /* ============================================================
     ...
     ============================================================ */
  function repasar() {
    if (!vistaViva) vistaViva = ruta();
    montarFab(); montarConfig();
  }

  /* 28/07/2026 · Al SALIR de una vista se borra su conversación entera.
     Decisión del usuario: al volver a entrar se arranca limpio, con
     "Iniciar" escrito, sin el hilo anterior colgando. Lo que se quiera
     conservar hay que exportarlo antes (Copiar / PDF / WhatsApp). */
  function olvidar(vista) {
    if (!vista) return;
    if (hilos[vista]) delete hilos[vista];
    if (abierta && cerrarHoja) cerrarHoja();   // la hoja abierta era de esa vista
  }

  window.addEventListener('hashchange', function () {
    Repro.parar();        // cambiar de vista calla al robot
    var ahora = ruta();
    if (ahora !== vistaViva) { olvidar(vistaViva); vistaViva = ahora; }
  ...
  window.__np11 = { trocear: Repro.trocear, esIniciar: esIniciar, VISTAS: VISTAS,
                    pintarError: pintarError, consultar: consultar, hilos: hilos,
                    montarConfig: montarConfig, contextoDe: contextoDe, olvidar: olvidar,
                    abrir: abrir, viva: function () { return vistaViva; },
                    /* 28/07: puerta para probar el híbrido sin navegador */
                    montarFab: montarFab, conInforme: conInforme, aHtml: aHtml,
