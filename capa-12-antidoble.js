/* ============================================================================
 * CAPA 12 · ANTI DOBLE CLIC  (app PRIVADA · JHONNY PRIV)   ·   31/07/2026
 * ----------------------------------------------------------------------------
 * Misma capa que ya está publicada en la app pública. Aquí va ajustada a este
 * repo: no toca style.css, no toca app.js y respeta el arrastre del robot de la capa 11.
 *
...
(function () {
  ...
  /* ---------------------------------------------------------------------
     ...
     --------------------------------------------------------------------- */
  try {
    var ESPERA = ['.btn', 'button', '.tile', '.chip', '.check', '.check-row', '.datebtn', '.hero', '.ag-cell', '.ag-dot', '.ag-dow', '.ag-pill', '.an-bar-row', '.an-row', '.an-check', '.cfg-acc-h', '.cfg-chip', '.cfg-sw', '.cfg-tab', '.ref-toggle', '.avatar-edit', '.iosp-btn', '.iosp-item', '.mark', '.lcard', '.rcard', '.link-quiet'];
    var reglas = '';
  ...
  /* --- parche del asignador `onclick` -------------------------------------
     ...
     (307 en app.js): quedan cubiertas de una. Lo poco que usa addEventListener
     (capa-11-insights.js y 2 sitios de app.js) lo cubre el escudo de pantalla. */
  var objetivos = [];
