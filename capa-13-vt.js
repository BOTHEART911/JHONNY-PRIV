/* ============================================================================
 * CAPA 13 · VIEW TRANSITIONS + CURVAS DE RESORTE — app PRIVADA   ·   02/08/2026
 * ----------------------------------------------------------------------------
 * QUÉ HACE
 *   Le pide al navegador que anime él mismo el cambio de pantalla (View
 *   Transitions API). Lo que se gana con eso:
 *     · el ELEMENTO COMPARTIDO: la tarjeta que tocas se transforma en la hoja
 *       de detalle, y la foto pequeña crece hasta la foto grande;
 *     · movimiento con CURVA DE RESORTE, como en las apps nativas.
 *
 * INSTALACIÓN (una sola línea, al final del <body>, DESPUÉS de capa-4-transicion.js)
 *   <script src="capa-13-vt.js"></script>
 *
 * PAREJA
 *   capa-13-vt.css  (obligatoria: sin ella esto no anima nada)
 *
 * CONVIVENCIA CON LA CAPA 4 (transición lateral)
 *   No se toca ni se borra la capa 4. Si el navegador soporta View
 *   Transitions, esta capa apaga EN CALIENTE su hoja de estilo
 *   (link[href*="capa-4-transicion"].disabled = true) y toma el mando. Su JS
 *   sigue corriendo sin efecto visible, y sigue marcando <html class="np-tx">,
 *   que es lo que mantiene callada a la capa 1.
 *   Si el navegador NO soporta View Transitions (iPhone con iOS 17 o menos),
 *   esta capa se retira en la primera línea y la capa 4 queda intacta.
 *
...
(function () {
  ...
  var CFG = {
    ORIGENES: ".bd-card, .tile, .an-card, .dash-card, .bot-card",
    ATRAS:    "#backbtn, #rf-back",
    TX:       "np-tx",
    ZOOM:     true
  };
  ...
  /* ---- 6 · ELEMENTO COMPARTIDO: foto pequeña -> foto grande ------------- */
  var zoomOrig = window.zoomImagen;
  if (CFG.ZOOM && typeof zoomOrig === 'function') {
    window.zoomImagen = function (src) {
      var self = this, args = arguments;
      var im = fotoTocada;
      if (!libre() || !src || !im || !im.isConnected) return zoomOrig.apply(self, args);
      marca(im, FOTO);
      transicion('jvt-zoom', function () {
        limpia(im);
        zoomOrig.apply(self, args);
        var ov = document.querySelector('#layer .imgzoom');
        /* .imgzoom nace en opacity:0 y se enciende en el rAF siguiente; si no
           lo encendemos aquí, la foto grande se fotografía invisible. Es la
           MISMA clase que se pone él solo, así que no cambia su conducta. */
        if (ov) { ov.classList.add('show'); marca(ov.querySelector('img'), FOTO); }
      }, function () {
        var ov = document.querySelector('#layer .imgzoom');
        if (ov) limpia(ov.querySelector('img'));
        limpia(im);
      });
    };
  }
