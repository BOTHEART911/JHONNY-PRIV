...
function rfMore() {
  ...
  tb.insertAdjacentHTML('beforeend', next.map((x, i) => `<tr>
      <td class="asis-i" data-l="#">${(RF_SHOWN + i + 1).toLocaleString('es-CO')}</td>
...
/* ---- MES: celdas parejas con puntos de estado + conteo ---- */
function agRenderMes() {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    const dots = res.map(x => `<span class="ag-dot ${x.clase}"><i></i>${x.n}</span>`).join('');
...
function agDayItemsHtml(rs) {
  return rs.map(r => `<button class="ag-day-item ${agEstadoClass(r.estado)} ${AG_SEL.has(String(r.id)) ? 'ag-picked' : ''}" data-id="${esc(r.id)}">
...
function agRenderDia() {
  ...
  rs.forEach(r => {
    if (hoy && !placed && agMinutos(r.inicia) >= nowMin) { out += now; placed = true; }
    out += agDayItemsHtml([r]);
  });
  if (hoy && !placed) out += now;
