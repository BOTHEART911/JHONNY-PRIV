...
function bdCardHtml(c) {
  ...
  return `<article class="bd-card ${bdColor(c.municipio)} ${inact ? 'bd-inact' : ''}" data-doc="${esc(c.documento)}">
    ...
    <div class="bd-acts">
      ...
      <button class="bd-btn bd-btn-reasg" data-a="reasignar">Reasignar</button>
...
function bdApplyFilter() {
  ...
  BD_FILT.sort((a, b) => bdRegMs(a) - bdRegMs(b));   // 21/07: de la más antigua a la más reciente (sort estable → empates conservan orden de hoja)
  BD_SHOWN = 0;
...
function bdUpsertCard(card, _prepend) {
  ...
  // 21/07: con orden por antigüedad, un registro NUEVO no se antepone: se
  // reordena y repinta para que caiga en su sitio (el más reciente, al final).
...
function bdAccion(a, doc) {
  ...
  if (a === 'reasignar') return bdReasignar(doc, card);
  if (a === 'inactivar') return bdInactivar(doc, card);
}

/* ---- REASIGNAR (21/07) · cambia el líder (REFERENCIA) de un registro ----
   Modal ligero: combo con búsqueda sobre BD_LIDERES (que ya llega en la
   carga) + Guardar. Solo se toca REFERENCIA; el binding real es por código
   (no por el texto LIDER). En vivo suena el canal 'bd'. */
function bdReasignar(doc, card) {
  card = card || BD_ALL.find(x => normDocJs(x.documento) === normDocJs(doc));
  const actual = card ? BD_LIDERES.find(l => String(l.codigo) === String(card.referencia || '')) : null;
  const actualTxt = actual ? bdLiderLabel(actual) : (card && card.referencia ? '#' + card.referencia : 'Sin líder asignado');
  openSheet(`<div class="grip"></div>
    <h2 class="h2">Reasignar líder</h2>
    <p class="muted small" style="margin:-4px 0 10px;">${esc((card && card.nombre) || '')} · CC ${esc(doc)}</p>
    <div class="reasg-cur"><span>Líder actual</span><b>${esc(actualTxt)}</b></div>
    <label class="field" style="margin-top:12px;"><span>Nuevo líder</span>${comboboxHtml('reasg-lid', 'Escribe para buscar líder…')}</label>
    <button class="btn btn-primary btn-block" id="reasg-save" style="margin-top:14px;">Guardar</button>`, 'reasg-sheet');
  bindCombobox('reasg-lid', BD_LIDERES.map(bdLiderLabel));
  ...
  $('#reasg-save').onclick = async () => {
    ...
    const code = m[1];
    if (String(code) === String((card && card.referencia) || '')) return toast('Ese ya es su líder', 'err');
    const btn = $('#reasg-save'); saving(btn, true);
    try {
      const r = await api('priv.personaReasignar', {}, 'POST', { ...auth(), documento: doc, referencia: code });
      saving(btn, false);
      if (!r.ok) return toast(r.msg || 'No se pudo reasignar', 'err');
      closeLayer(); toast('Registro reasignado', 'ok'); bdUpsertCard(r.card);
...
function agTplRender(body, vars) {
  const out = String(body || '').replace(/\{(\w+)\}/g, (m, k) => (vars[k] != null ? vars[k] : ''));
...
function cfPintaGeneral() {
  ...
  const extraSec = otras.length ? [{
    titulo: 'Otros ajustes', sub: 'Claves de la hoja CONFIG que no tienen un sitio propio.',
    inner: `<div class="cfg-grid">${otras.map(k => cfField('cf-k-' + k, k, cfg[k], (CF.d.meta[k] || {}).descripcion, true)).join('')}</div>`
  }] : [];
