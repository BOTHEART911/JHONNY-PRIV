...
const READY = new Set(['bd', 'lideres', 'eventos', 'agenda', 'compromisos', 'notificaciones', 'analisis', 'config']);
...
function anComCards(items) {
  ...
  return items.map(it => `
    <article class="an-com ${it.visible ? '' : 'off'}">
      <div class="an-com-media">
        ${it.imagen ? `<img src="${esc(it.imagen)}" alt="" loading="lazy" onerror="this.closest('.an-com-media').classList.add('noimg')">` : ''}
...
const CF_GENERAL = [
  { titulo: 'Enlaces', sub: 'A dónde llevan los botones de la app pública.', keys: [
    ['REFIERE_URL', 'Enlace para invitar', 'Se manda en "Refiere por WhatsApp".'],
    ['COMERCIO_URL', 'App de comerciantes', ''],
    ['REGISTRADURIA_URL', 'Consultar puesto de votación', ''],
    ['APP_PUBLICA_URL', 'URL de la app pública', 'Destino del push de "Ponte al día".']
  ] },
  { titulo: 'Imágenes y marca', sub: 'Pega la URL de la imagen.', keys: [
    ...
    ['INICIO_IMAGEN_DRIVE', 'Imagen de inicio', 'ID o URL de Drive.'],
    ['INICIO_REEL_YT', 'Reel de YouTube (inicio)', '']
  ] },
...
const CF_AV_KEYS = ['BUILDERBOT_URL', 'BUILDERBOT_APIKEY', 'DRIVE_QR_FOLDER', 'DRIVE_CASA_FOLDER',
...
function cfPintaGeneral() {
  ...
  CF_CASA_KEYS.concat(CF_EM_KEYS, CF_AV_KEYS).forEach(k => usadas[k] = 1);
  ...
  const keys = CF_GENERAL.reduce((a, g) => a.concat(g.keys.map(k => k[0])), []).concat(otras);
  $('#cf-body').innerHTML = grupos + extra + `<div class="cfg-actions"><button class="btn btn-primary" id="cf-gen-save">Guardar cambios</button></div>`;
...
function cfPintaCasa() {
  ...
  const textos = cfCard('Textos de la vista', 'Lo que lee la gente en la app pública.',
    `<div class="cfg-grid">
      ...
      ${cfField('cf-k-CASA_LAT', 'Latitud', cfg.CASA_LAT, 'Para el botón "Iniciar recorrido".')}
      ${cfField('cf-k-CASA_LNG', 'Longitud', cfg.CASA_LNG, '')}
  ...
  $('#cf-body').innerHTML = CF.d.plantillas.map((p, i) => cfCard(p.titulo,
    ...
    `<div class="cfg-chips">${p.vars.map(v => `<button class="cfg-chip" data-v="${esc(v)}" data-i="${i}">${esc(v)}</button>`).join('')}</div>
     ...
     <div class="cfg-hint">${p.sintaxis === 'corchetes'
        ? 'Sintaxis con corchetes. <b>%0A</b> es un salto de línea: déjalos donde quieras que el mensaje baje una línea.'
        : 'Sintaxis con llaves. Los saltos de línea van tal cual (Enter).'}</div>
...
/* Muestra el mensaje como le llega a la persona: los %0A pasan a saltos reales */
function cfTplPrev(i) {
  ...
  if (p.sintaxis === 'corchetes') out = out.split('%0A').join('\n');
...
function cfUserSheet(us) {
  ...
  onlyDigits($('#cf-us-d')); onlyDigits($('#cf-us-p'));
  if (us) {
  ...
  $('#cf-us-save').onclick = async e => {
    ...
    try {
      const r = await api('priv.cfgUsuario', {}, 'POST', Object.assign(cfAuth(), {
        ...
        correo: val('cf-us-c'), rol: val('cf-us-r'), pin: val('cf-us-p'), estado: val('cf-us-e')
      }));
      saving(e.currentTarget, false);
      if (!r.ok) return toast(r.msg || 'No se pudo', 'err');
...
function cfPintaAvanzado() {
  ...
  const bot = cfCard('BuilderBot', 'El bot de WhatsApp que usan Líderes, Compromisos, Agenda y Servicios.',
    `<div class="cfg-estados">${est(a.bot.url, 'Endpoint configurado', 'Falta el endpoint')}${est(a.bot.apikey, 'API key guardada', 'FALTA la API key: sin ella el bot no envía')}</div>
     <div class="cfg-grid">
      ${cfField('cf-k-BUILDERBOT_URL', 'Endpoint (POST)', cfg.BUILDERBOT_URL, 'https://app.builderbot.cloud/api/v2/…/messages', true)}
      ${cfField('cf-k-BUILDERBOT_APIKEY', 'API key', cfg.BUILDERBOT_APIKEY, 'Se manda en el header x-api-builderbot.', true)}
     </div>
     <div class="cfg-actions">
  ...
  $('#cf-body').innerHTML = bot + infra;
  $('#cf-bot-save').onclick = e => cfGuardarClaves(e.currentTarget, ['BUILDERBOT_URL', 'BUILDERBOT_APIKEY']);
  $('#cf-inf-save').onclick = e => cfGuardarClaves(e.currentTarget, ['DRIVE_QR_FOLDER', 'DRIVE_CASA_FOLDER', 'CAL_EVENTOS', 'CAL_AGENDA']);
