...
function agRenderMes() {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    const chips = rs.slice(0, 3).map(r => `<button class="ag-chip ${agEstadoClass(r.estado)} ${AG_SEL.has(String(r.id)) ? 'ag-picked' : ''}" data-id="${esc(r.id)}" title="${esc(r.titulo)}">${r.inicia ? `<span class="ag-chip-h">${esc(hora12(r.inicia))}</span> ` : ''}${esc(r.titulo)}</button>`).join('');
...
function agRenderDia() {
  ...
  const items = rs.map(r => `<button class="ag-day-item ${agEstadoClass(r.estado)} ${AG_SEL.has(String(r.id)) ? 'ag-picked' : ''}" data-id="${esc(r.id)}">
    <div class="ag-day-time">${esc(r.inicia ? hora12(r.inicia) : '—')}${r.termina ? `<span>${esc(hora12(r.termina))}</span>` : ''}</div>
