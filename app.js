...
function agRenderMes() {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    const chips = rs.slice(0, 3).map(r => `<button class="ag-chip ${agEstadoClass(r.estado)} ${AG_SEL.has(String(r.id)) ? 'ag-picked' : ''}" data-id="${esc(r.id)}" title="${esc(r.titulo)}">${r.inicia ? `<span class="ag-chip-h">${esc(r.inicia)}</span> ` : ''}${esc(r.titulo)}</button>`).join('');
...
function agWireMonth() {
  $$('#ag-cal .ag-chip').forEach(b => b.onclick = (e) => { e.stopPropagation(); AG_SELMODE ? agSelToggle(b.dataset.id) : agVer(b.dataset.id); });
  ...
  $$('#ag-cal .ag-cell').forEach(c => c.onclick = () => { if (AG_SELMODE) return; const d = parseFechaES(c.dataset.day); if (d) agAgregar(d); });
}

function agRenderDia() {
  const rs = agReunionesDia(AG_CUR);
  if (!rs.length) return `<div class="ag-daywrap"><p class="muted center" style="margin:30px 0;">${AG_EST.size ? 'Sin reuniones de ese estado este día.' : 'Sin reuniones este día.'}<br><button class="btn btn-ghost" id="ag-day-add" style="margin-top:12px;">${I.plus} Agregar reunión</button></p></div>`;
  const items = rs.map(r => `<button class="ag-day-item ${agEstadoClass(r.estado)} ${AG_SEL.has(String(r.id)) ? 'ag-picked' : ''}" data-id="${esc(r.id)}">
...
async function vivoCompromisos() {
  ...
  cmPintarResumen(); cmApplyFilter(); vivoCerrarHoja();
}
...
const CF_TABS = [
  ...
  { k: 'usuarios', label: 'Usuarios', dev: true },
  { k: 'call', label: 'Call Center', dev: true },
...
const CF_GENERAL = [
  { titulo: 'Otros enlaces', sub: 'A dónde llevan otros botones de la app pública.', keys: [
    ['REFIERE_URL', 'Enlace para invitar', 'Se manda en "Refiere por WhatsApp".'],
...
const CF_OCULTAS = ['INICIO_IMAGEN_ID', 'ALERTA_BOT_FALLOS', 'ALERTA_BOT_ULTIMA_ALERTA', 'BOT_SILENCIADO'];
...
async function vivoConfig() {
  const d = await api('priv.cfgTodo', cfAuth(), 'GET', null, SILENCIO);
...
async function cfCargar() {
  try {
    CF.d = await api('priv.cfgTodo', cfAuth());
    cfRender();
  } catch (e) {
...
function cfCard(titulo, sub, inner) {
  return `<div class="cfg-card"><h3 class="cfg-card__title">${esc(titulo)}</h3>${sub ? `<p class="cfg-card__sub">${esc(sub)}</p>` : ''}${inner}</div>`;
}
...
function cfPintaGeneral() {
  const cfg = CF.d.cfg;
  const usadas = {};
  CF_GENERAL.forEach(g => g.keys.forEach(k => usadas[k[0]] = 1));
  CF_APPS.forEach(a => usadas[a[0]] = 1);
  ...
  const grupos = CF_GENERAL.map(g => cfCard(g.titulo, g.sub,
    `<div class="cfg-grid">${g.keys.map(k => cfField('cf-k-' + k[0], k[1], cfg[k[0]], k[2], true)).join('')}</div>`)).join('');

  const extra = otras.length ? cfCard('Otros ajustes', 'Claves de la hoja CONFIG que no tienen un sitio propio.',
    `<div class="cfg-grid">${otras.map(k => cfField('cf-k-' + k, k, cfg[k], (CF.d.meta[k] || {}).descripcion, true)).join('')}</div>`) : '';

  const keys = CF_APPS.map(a => a[0])
    .concat([CF_COMPARTIR_KEY])
    .concat(CF_GENERAL.reduce((a, g) => a.concat(g.keys.map(k => k[0])), []))
    .concat(otras, ['INICIO_REEL_URL']);
  $('#cf-body').innerHTML = cfAppsCard() + grupos + cfInicioCard() + extra +
...
/* ---------- Enlaces de las 5 apps + plantilla "Compartir App" (17/07/2026) ----------
   Un botón Copiar por app: copia el MENSAJE ya armado (no el enlace pelado),
   listo para pegar en WhatsApp. Lee lo que hay EN EL CAMPO, no lo guardado:
   si acabas de corregir un enlace y copias sin guardar, se copia lo que ves. */
function cfAppsCard() {
  const cfg = CF.d.cfg;
  const filas = CF_APPS.map(a => `<div class="cfg-field full">
      ...
      <div class="cfg-copyrow">
        ...
        <button type="button" class="btn btn-quiet cfg-copy" id="cf-cp-${a[0]}" data-k="${a[0]}" data-n="${esc(a[3])}" title="Copiar el mensaje para compartir esta app">Copiar</button>
      </div>
      ${a[2] ? `<div class="cfg-hint">${esc(a[2])}</div>` : ''}
    </div>`).join('');

  return cfCard('Enlaces de las apps', 'Las 6 apps del proyecto. El botón Copiar arma el mensaje de abajo con el nombre y el enlace de esa app.',
    `<div class="cfg-grid">${filas}
       ${cfArea('cf-k-' + CF_COMPARTIR_KEY, 'Plantilla rápida · Compartir App', cfg[CF_COMPARTIR_KEY],
          'Variables: {nombreApp} y {enlaceApp}. Es lo que se copia con cada botón Copiar.', 4)}
     </div>
     `);
}
...
function cfAppsBind() {
  CF_APPS.forEach(a => {
    ...
    b.onclick = () => {
      ...
      copiarTexto(cfCompartirTexto(a[3], enlace), 'Mensaje de la App ' + a[3] + ' copiado');
    };
  });
}
...
function cfInicioCard() {
  ...
  return cfCard('Inicio de la app pública', 'La imagen y el video que ve la gente al abrir la app.',
    `<div class="cfg-grid">
      ...
      <div class="cfg-day${d.activo ? ' on' : ''}">
        ...
        <div class="cfg-day-b">
          ${d.items.length ? `<ul class="cfg-items">${d.items.map(i => `
            <li class="cfg-item">
              <div class="cfg-item-t"><b>${esc(i.item)}</b>${i.horario ? `<span class="cfg-item-h">${I.clock} ${esc(i.horario)}</span>` : ''}</div>
...
function cfItemSheet(dia, item) {
  ...
  openSheet(`<div class="grip"></div>
    ...
    <div class="stack" style="margin-top:14px;">
      ...
      <p class="small muted">El horario es texto libre y puede quedar vacío.</p>
  ...
  $('#cf-it-save').onclick = async e => {
    ...
    try {
      const r = await api('priv.cfgCasaItem', {}, 'POST', Object.assign(cfAuth(), {
        id: item ? item.id : '', dia, item: item2, horario: val('cf-it-hor')
      }));
...
/* ============================================================
   USUARIOS  (solo DEV · el DESARROLLADOR lleva candado)
   ============================================================ */
function cfPintaUsuarios() {
  const u = CF.d.usuarios;
  $('#cf-body').innerHTML = cfCard('Usuarios de la app privada', 'Quién entra al panel y con qué rol.',
      ...
      <li class="cfg-row">
        ...
        <div class="cfg-row-a">
          ...
          ${x.esDev ? '<span class="cfg-pill cfg-pill-dev">Protegido</span>' : `
            ...
            <button class="bd-btn bd-btn-danger" data-usdel="${esc(x.documento)}" title="Eliminar">${I.trash}</button>`}
        </div>
      </li>`).join('')}</ul>
...
function cfCallUsuariosCard() {
  ...
  const filas = us.length ? `<ul class="cfg-list">${us.map(x => `
    <li class="cfg-row">
      ...
      <div class="cfg-row-a">
        ${x.estado === 'ACTIVO' ? '' : '<span class="cfg-pill">Inactivo</span>'}
        <button class="bd-btn" data-cu="${esc(x.id)}" title="Editar">${I.pencil}</button>
        <button class="bd-btn bd-btn-danger" data-cudel="${esc(x.id)}" title="Eliminar">${I.trash}</button>
      </div>
  ...
  const cuerpo = CFCALL.dias.length ? CFCALL.dias.map(d => `
    <div class="cc-dia" data-d="${esc(d.dia)}">
      <div class="cc-dia-h">
        ...
        <button class="bd-btn bd-btn-danger" data-ddel="${esc(d.dia)}" title="Quitar el día">${I.trash}</button>
      </div>
      <div class="cc-horas">
      ...
      ${d.usuarios.length ? `<ul class="cfg-list">${d.usuarios.map(u => `
        <li class="cfg-row">
          ...
          <div class="cfg-row-a">
            <button class="bd-btn" data-uld="${esc(d.dia)}" data-ulu="${esc(u.id)}" title="Líderes que trabajará">${I.sliders}</button>
            <button class="bd-btn bd-btn-danger" data-udel="${esc(d.dia)}" data-uduu="${esc(u.id)}" title="Quitar del día">${I.x}</button>
          </div>
...
function cfPintaAvanzado() {
  ...
  $('#cf-body').innerHTML = cfEleccionCard() + bot + infra;
