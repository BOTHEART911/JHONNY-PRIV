...
function menuParaRol(rol) {
  ...
  if (r === 'SEDE') return MENU.filter(m => m.id === 'mibot');
  ...
  app.innerHTML = `${appbar(user)}
    <div class="pad stack">
      <div>
        ...
        <span class="rol-badge ${rol === 'DESARROLLADOR' ? 'dev' : rol === 'ADMIN' ? 'admin' : rol === 'INVITADO' ? 'inv' : 'sede'}">${esc(rolLabel(rol))}</span>
      </div>
...
function cfUserSheet(us) {
  const roles = [['ADMIN', 'ADMIN — ve todo el panel'], ['SEDE', 'SEDE — solo Mi Bot'],
                 ['INVITADO', 'INVITADO — solo lo que le marques abajo']];
  openSheet(`<div class="grip"></div>
...
/* ============================================================
   EDITOR DE PASTILLAS DEL ROL INVITADO  (06/08/2026)
   ------------------------------------------------------------
   El catálogo lo manda el CORE (priv.permCatalogo · Permisos.gs): aquí NO
   hay ninguna lista escrita a mano, para que no se separen nunca. Se pide
   una sola vez por sesión y se guarda en CF_PERM_CAT.

   Los 4 CANDADOS de arriba apagan una familia entera (exportar, eliminar,
   WhatsApp, IA). Ganan siempre: la pastilla se sigue viendo marcada pero
   queda en gris y no cuenta, así que se puede quitar el candado y recuperar
   lo que ya estaba elegido sin volver a marcarlo.
   ============================================================ */
...
function cfPermTapada(it) { return !!(it.fam && CF_PERM_EDIT.x.indexOf(it.fam) !== -1); }

function cfPermChip(it) {
  const on = CF_PERM_EDIT.c.indexOf(it.k) !== -1;
...
function cfPermCandLabel(fam) {
  const c = ((CF_PERM_CAT && CF_PERM_CAT.candados) || []).find(x => x.k === fam);
...
function cfPermPintar() {
  ...
  const vistas = (CF_PERM_CAT.catalogo || []).map(v => {
    const chips = (v.items || []).map(cfPermChip).join('');
    const grupos = (v.grupos || []).map(g => `<div class="perm-grupo">
        <span class="perm-glabel">${esc(g.label)}</span>
        <div class="perm-chips">${(g.items || []).map(cfPermChip).join('')}</div>
      </div>`).join('');
    return `<section class="perm-vista">
      <header class="perm-vhead">
        <b>${esc(v.label)}</b>
  ...
  box.innerHTML = `<div class="perm-head">
    ...
    <p class="muted small">Marca lo que podrá ver y hacer. Los candados de abajo apagan una familia entera de una vez.</p>
  ...
  box.querySelectorAll('[data-todo]').forEach(b => b.onclick = () => { cfPermVista(b.dataset.todo, true); cfPermPintar(); });
  box.querySelectorAll('[data-nada]').forEach(b => b.onclick = () => { cfPermVista(b.dataset.nada, false); cfPermPintar(); });
  cfPermCuenta();
}

/* Marca o desmarca una vista entera (con sus secciones) */
function cfPermVista(id, on) {
  const v = (CF_PERM_CAT.catalogo || []).find(x => x.id === id); if (!v) return;
  const claves = [];
  (v.items || []).forEach(it => claves.push(it.k));
  (v.grupos || []).forEach(g => (g.items || []).forEach(it => claves.push(it.k)));
  claves.forEach(k => {
    const i = CF_PERM_EDIT.c.indexOf(k);
    if (on && i === -1) CF_PERM_EDIT.c.push(k);
    if (!on && i !== -1) CF_PERM_EDIT.c.splice(i, 1);
  });
}
...
function cfPermCuenta() {
  ...
  el.textContent = n ? `${n} pastilla${n === 1 ? '' : 's'} activa${n === 1 ? '' : 's'}` : 'Sin permisos: no se puede guardar';
