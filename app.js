...
function abrirRuedaFecha(iso, onOk, opts) {
  ...
  IOSP.anios = [];
  if (IOSP.conAnio) {
    const desde = Math.min(2019, d.getFullYear());
    for (let y = desde; y <= hoy.getFullYear() + 1; y++) IOSP.anios.push(y);
    if (IOSP.anios.indexOf(IOSP.anio) === -1) IOSP.anio = hoy.getFullYear();
  }
...
function campoFecha(id, label, opts) {
  ...
  return `<div class="field">
      <span>${esc(label)}</span>
      <button type="button" class="input datebtn" id="${id}-btn" data-minhoy="${o.minHoy ? 1 : 0}" data-anios="${o.anios ? 1 : 0}">
...
async function vivoSimulador() {
  ...
  if (!r.ok || !vivoCambio(JSON.stringify(r.puestos))) return;
...
async function evDetalles(id) {
  ...
  openSheet(`<div class="grip"></div>
    ...
    <section class="asis-sec" id="ev-asis-sec">
      <div class="asis-head">
        <h3 class="asis-t">${I.users} Asistentes <span class="asis-n" id="ev-asis-n">—</span></h3>
...
const CF_GENERAL = [
  ...
  { titulo: '🔔 Avisos del bot', sub: 'Quién recibe el correo si el bot de WhatsApp se desconecta. Se revisa cada hora y solo avisa tras dos chequeos caídos seguidos.', keys: [
    ...
    ['APP_PRIVADA_URL', 'URL de la app privada', 'Destino del botón del correo de aviso.']
  ] }
];
...
function cfPintaGeneral() {
  ...
  const keys = CF_GENERAL.reduce((a, g) => a.concat(g.keys.map(k => k[0])), []).concat(otras, ['INICIO_REEL_URL']);
  $('#cf-body').innerHTML = grupos + cfInicioCard() + extra +
