...
function cfAcc(secciones) {
  return `<div class="cfg-acc" id="cf-acc">${secciones.map((s, i) => `
    <section class="cfg-acc-sec${i === 0 ? ' open' : ''}">
      <button type="button" class="cfg-acc-h" aria-expanded="${i === 0 ? 'true' : 'false'}">
...
function cfAccBind() {
  $$('#cf-acc .cfg-acc-h').forEach(h => h.onclick = () => {
    const sec = h.closest('.cfg-acc-sec');
    const abierta = sec.classList.contains('open');
    $$('#cf-acc .cfg-acc-sec').forEach(s => { s.classList.remove('open'); const b = s.querySelector('.cfg-acc-h'); if (b) b.setAttribute('aria-expanded', 'false'); });
