...
let LD_COLS = [];
const LD_COLS_FALLBACK = ['CODIGO', 'NOMBRE', 'DOCUMENTO', 'CONTACTO', 'RESIDENCIA', 'PADRINO', 'REFERIDOS', 'CORREO', 'CONTRASEÑA'];
const LD_COLS_DEFECTO = ['CODIGO', 'NOMBRE', 'DOCUMENTO', 'CONTACTO', 'RESIDENCIA', 'PADRINO', 'REFERIDOS'];
const LD_BATCH = 60;
...
function ldExportar() {
  const cods = LD_FILT.map(c => String(c.codigo == null ? '' : c.codigo).trim()).filter(Boolean);
  if (!cods.length) return toast('No hay líderes en el filtro actual', 'err');
  ...
  openSheet(`<div class="grip"></div>
    ...
    <p class="muted small">${cods.length.toLocaleString('es-CO')} líder(es) · ${esc(filtroTxt)}. Elige las columnas:</p>
    <div class="ldx-tools">
  ...
  const bajar = async (tipo, btn) => {
    ...
    try {
      const r = await api(tipo === 'excel' ? 'priv.lideresExcel' : 'priv.lideresPdf', {}, 'POST',
        { caller: LD_ME, codigos: cods, columnas, filtroTxt });
      saving(btn, false);
      if (!r.ok) return toast(r.msg || 'No se pudo generar', 'err');
      downloadB64(r.base64, r.mime, r.filename);
      closeLayer(); toast('Archivo generado', 'ok');
