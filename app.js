...
function ldClaseChip(c) {
  ...
  return `<div class="ld-clase-row"><span class="ld-clase" title="Clasificaci\u00F3n del l\u00EDder"><i>${k.emoji}</i>${esc(k.etiqueta)}</span></div>`;
}
...
function ldCardHtml(c) {
  ...
  return `<article class="bd-card ld-card ${ldColor(c.referidos)}" data-cod="${esc(c.codigo)}">
    ...
    <div class="ld-act-row">${ldActivadoHtml(c)}</div>
